...
class ArticleController {
  static async getArticles(req, res, next) {
    try {
      const { search, categoria, minNota, page = 1, limit = 10 } = req.query;
      const resultado = await artigoService.listar({ search, categoria, minNota, page, limit });
  ...
  static async updateArticle(req, res, next) {
    try {
      const id = parseInt(req.params.id, 10);
      const { titulo, conteudo, resumo, categoria, imageUrl } = req.body;
      const artigo = await artigoService.atualizar(id, { titulo, conteudo, resumo, categoria, imageUrl }, req.user.id);
